    ...
    def beam_search_test(self, h_word, init_logprobs, temp_att, temp_theta, h_t_1, opt):
        ...
        def beam_step(logprobsf, unaug_logprobsf, beam_size, t, beam_seq, beam_seq_logprobs, beam_logprobs_sum, h_word):
            ...
            for vix in range(beam_size):
                ...
                # rearrange recurrent states
                #  copy over state in previous beam q to new beam at vix
                new_h_word[:, vix,:] = h_word[:, v['q'],:]  # dimension one is time step
        ...
        # logprobs # logprobs predicted in last time step, shape (beam_size, vocab_size+1)
        done_beams_table = [[] for _ in range(group_size)]
        # h_word_table = [list(torch.unbind(_)) for _ in torch.stack(h_word).chunk(group_size, 2)]
        # c_word_table = [list(torch.unbind(_)) for _ in torch.stack(c_word).chunk(group_size, 2)]
        logprobs_table = list(init_logprobs.chunk(group_size, 0))
        # END INIT

        # Chunk elements in the args
        # args = [temp_att, temp_theta]
        # args = [_.chunk(group_size) if _ is not None else [None] * group_size for _ in args]
        # args = [[args[i][j] for i in range(len(args))] for j in range(group_size)]
